...
option_list <- list(
  ...
  make_option(c("--color_column"), type="character", default='RFdiffusion',
  ...
  make_option(c("--title"), type="character", default="Expressed",
  ...
  make_option(c("--subtitle"), type="character", default="Number of expressed designs per round",
              help="Custom subtitle for the plot [default=%default]"),
  make_option(c("--sort"), type="character", default="alpha",
  ...
  tryCatch({
    ...
    p <- ggplot(plot_data) +
      ...
      geom_text(data = counts,
               ...
               vjust = -0.5,
               color = "#3D7A9A",
               size = 4,  # Decreased text size from 6 to 4
               fontface = "bold") +
      scale_fill_manual(values = color_values,
                      name = stringr::str_to_sentence(gsub("_", " ", opt$color_column))) +
      labs(title = title,
           subtitle = subtitle,
